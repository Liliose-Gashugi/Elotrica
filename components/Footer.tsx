const WA_LINK = "https://wa.me/250788000000?text=Hello%20Elotrica%2C%20I%20would%20like%20to%20book%20a%20ride.";

const nav = [
  { label: "Services", href: "#services" },
  { label: "Fleet", href: "#fleet" },
  { label: "Pricing", href: "#pricing" },
  { label: "About", href: "#why" },
  { label: "How It Works", href: "#how" },
  { label: "Testimonials", href: "#testimonials" },
];

export default function Footer() {
  return (
    <footer id="footer" className="bg-[#071510] text-[#f7f4ef]">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">

        {/* Top divider */}
        <div className="h-px bg-gradient-to-r from-transparent via-[#A88549]/20 to-transparent" />

        <div className="py-14 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">

          {/* Brand column */}
          <div className="lg:col-span-2">
            <a href="#hero" className="flex items-center gap-2.5 mb-5 group w-fit">
              <div className="w-8 h-8 rounded-lg bg-[#A88549] flex items-center justify-center group-hover:scale-105 transition-transform">
                <svg className="w-4 h-4 text-white" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M13 10V3L4 14h7v7l9-11h-7z" />
                </svg>
              </div>
              <span className="text-xl font-bold text-[#f7f4ef]" style={{ fontFamily: "var(--font-playfair), serif" }}>
                ELOTRICA
              </span>
            </a>

            <p className="text-sm text-[#f7f4ef]/40 max-w-[280px] leading-relaxed mb-7">
              Rwanda&apos;s premium electric vehicle transportation service. Luxury travel. Zero emissions. 24/7.
            </p>

            {/* Contact list */}
            <div className="space-y-3">
              <a href={WA_LINK} target="_blank" rel="noopener noreferrer"
                className="flex items-center gap-3 text-sm text-[#f7f4ef]/50 hover:text-[#2d7a60] transition-colors group/link">
                <div className="w-7 h-7 rounded-lg bg-[#2d7a60]/15 flex items-center justify-center flex-shrink-0 group-hover/link:bg-[#2d7a60]/25 transition-colors">
                  <svg className="w-3.5 h-3.5 text-[#2d7a60]" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                  </svg>
                </div>
                +250 780 498 807
              </a>
              <a href="mailto:info@elotrica.rw"
                className="flex items-center gap-3 text-sm text-[#f7f4ef]/50 hover:text-[#A88549] transition-colors group/link">
                <div className="w-7 h-7 rounded-lg bg-[#A88549]/12 flex items-center justify-center flex-shrink-0 group-hover/link:bg-[#A88549]/20 transition-colors">
                  <svg className="w-3.5 h-3.5 text-[#A88549]" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                  </svg>
                </div>
                info@elotrica.rw
              </a>
              <div className="flex items-center gap-3 text-sm text-[#f7f4ef]/40">
                <div className="w-7 h-7 rounded-lg bg-[#f7f4ef]/5 flex items-center justify-center flex-shrink-0">
                  <svg className="w-3.5 h-3.5 text-[#f7f4ef]/40" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.8}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M17.657 16.657L13.414 20.9a2 2 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                  </svg>
                </div>
                Kigali, Rwanda · Daily 5AM – 11PM
              </div>
            </div>
          </div>

          {/* Navigation */}
          <div>
            <h4 className="text-[11px] font-bold tracking-[0.25em] uppercase text-[#f7f4ef]/30 mb-5">Navigation</h4>
            <ul className="space-y-3">
              {nav.map((l) => (
                <li key={l.href}>
                  <a href={l.href}
                    className="text-sm text-[#f7f4ef]/45 hover:text-[#A88549] transition-colors">
                    {l.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Services */}
          <div>
            <h4 className="text-[11px] font-bold tracking-[0.25em] uppercase text-[#f7f4ef]/30 mb-5">Services</h4>
            <ul className="space-y-3">
              {["Corporate VIP", "Individual & Leisure", "Visitor & Tourist", "Airport Transfers", "Multi-day Tours"].map((s) => (
                <li key={s}>
                  <span className="text-sm text-[#f7f4ef]/40">{s}</span>
                </li>
              ))}
            </ul>
          </div>
        </div>

        {/* Bottom bar */}
        <div className="h-px bg-[#f7f4ef]/5" />
        <div className="py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-xs text-[#f7f4ef]/20">
            © {new Date().getFullYear()} Elotrica Ltd. All rights reserved.
          </p>
          <div className="flex items-center gap-3">
            <a href="#" className="text-xs text-[#f7f4ef]/25 hover:text-[#f7f4ef]/50 transition-colors">Privacy Policy</a>
            <span className="text-[#f7f4ef]/15 text-xs">·</span>
            <a href="#" className="text-xs text-[#f7f4ef]/25 hover:text-[#f7f4ef]/50 transition-colors">Terms &amp; Conditions</a>
          </div>
          <div className="flex items-center gap-2 text-[10px] text-[#f7f4ef]/18 tracking-wide">
            <span className="w-1.5 h-1.5 rounded-full bg-[#2d7a60] opacity-60" />
            100% Electric · Kigali, Rwanda
          </div>
        </div>
      </div>
    </footer>
  );
}
