...
const BOOKING_EMAIL = "info@elotrica.com";
...
// Gmail compose link (opens Gmail web/app) with the same pre-filled message
const gmailLink = (service?: string) => {
  const svc = service ? encodeURIComponent(service) : "(Corporate%20%2F%20Individual%20%2F%20Tourist)";
  const body =
    `Hello%20Elotrica%2C%0A%0A` +
    `I%20would%20like%20to%20book%20a%20ride.%20Here%20are%20my%20details%3A%0A%0A` +
    `Name%3A%20%0A` +
    `Date%20%26%20Time%3A%20%0A` +
    `Pickup%20Location%3A%20%0A` +
    `Dropoff%20Location%3A%20%0A` +
    `Passengers%3A%20%0A` +
    `Service%20Type%3A%20${svc}%0A%0A` +
    `Thank%20you!`;
  return `https://mail.google.com/mail/?view=cm&fs=1&to=${BOOKING_EMAIL}&su=Booking%20Request%20%E2%80%94%20Elotrica&body=${body}`;
};
...
function BookingModal({ onClose, service }: { onClose: () => void; service?: string }) {
  ...
  return (
    ...
    >
      <div
        className="relative bg-white rounded-3xl p-7 w-full max-w-sm shadow-2xl my-auto"
      ...
      >
        ...
        {/* Email → opens Gmail compose with pre-filled message */}
        <a
          href={gmailLink(service)}
          target="_blank"
          rel="noopener noreferrer"
          onClick={onClose}
          className="group flex items-center gap-4 p-4 rounded-2xl border-2 border-[#A88549]/20 hover:border-[#A88549] hover:bg-[#A88549]/5 transition-all"
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-[#122820]/45 mt-0.5">Opens Gmail, ready to send</div>
          </div>
...
export default function BookingButton({
  ...
  return (
    <>
      ...
      {open && mounted &&
        createPortal(
          <BookingModal onClose={() => setOpen(false)} service={service} />,
          document.body
        )}
    </>
  );
